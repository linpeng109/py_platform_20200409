...
class RightWidget(QWidget):
    def __init__(self, config, logger, ports, callback_func):
        ...
        # select_widget
        select_widget = QPushButton('切换')
        language_choice = ['en', 'cn']
        # callback_func = lambda id: print(language_choice[id])
        choice_dialog = ChoiceDialog(choices=language_choice, title='选择语言', callback=callback_func)
        select_widget.clicked.connect(lambda event: choice_dialog.show())
